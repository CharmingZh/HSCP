...
Data = xlsread('after_400+_20_2_2.xlsx');
Label = Data(:, 1);
Data = Data(:, 2:end);
[trainData, trainLabel, testData, testLabel] = splitData(Data, Label, 0.7);
for z=1:100
    %[trainData, trainLabel, testData, testLabel] = splitData(Data, Label, 0.5);
    
% Wtrain=xlsread('Wtrain.xlsx');
Wtrain = [trainLabel, trainData];

T=Wtrain(:,1)';
P=Wtrain(:,2:size(Wtrain,2))'; 

% Wtest=xlsread('Wtest.xlsx');
Wtest = [testLabel, testData];
